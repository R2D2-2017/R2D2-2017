/**
* \file      patnode.hh
* \brief     PathNode inherits Node and adds extra variables necessary to
...
private:
     ...
     *
     * pathDistance is the total weight of all vertices passed when going to
     * this node from the starting node pathDistance changes depending on what
     * path is taken towards this node
     */
    float pathDistance;
     ...
     *
     * priority shows how important this node is to the path, lower priority is
     * more important priority is calculated on object construction and when the
     * parent is set. This makes sure that priority is always correct
     */
    float priority = 0;

    /**
     * \brief parent is a shared_ptr<PathNode> to the node with the shortest g
     *        result.
     *
     * Parent is a shared_ptr<PathNode> which points to the node the algorithm
     * came from before visiting this node parent can change when this node is
     * encountered from a different node where this nodes g would be smaller
     */
...
public:
     ...
     *
     * The constructor sets the PathNodes name and coordinates based on the
     * original node of which it is a copy.
     *
    ...
    /**
     * \brief Gets the objects pathDistance.
     *
    ...
    /**
     * \brief Gets the priority of a pathnode.
     *
    ...
    /**
     * \brief Puts pathnode in ostream.
     *
    ...
    /**
     * \brief Puts pathnode in sf::Packet.
     *
    ...
    /**
     * \brief Gets pathnode from sf::Packet
     *
...
/**
 * \brief Puts pathnode vector in sf::Packet
 *
...
/**
 * \brief Gets pathnode vector from sf::Packet
 *
