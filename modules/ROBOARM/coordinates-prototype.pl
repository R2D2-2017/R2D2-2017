...
sub dump_arm {
    my %arm = @_;
    say "points at " . fmtpos(@{{ arm_get_positions %arm }->{point1}}) . ", ",
                       fmtpos(@{{ arm_get_positions %arm }->{point2}});
}

sub show_arm {
    my %arm = @_;

    my $scale = 4; # Must be at least 2 to accommodate the font.
    ...
    for my $r (0..10) {
        for my $c (0..10) {
            #$im->setPixel($r*10*$scale, $c*10*$scale, $grey);
            $im->filledRectangle($r*10*$scale,        $c*10*$scale,
                                 $r*10*$scale+$scale, $c*10*$scale + $scale,
                                 $grey);
        }
    }
    ...
    $im->string(gdGiantFont, 2, 0, "ends at", $black);
    $im->string(gdGiantFont, 2, 14, fmtpos(@{$pos{point1}}), $red);
    $im->string(gdGiantFont, 2, 28, fmtpos(@{$pos{point2}}), $green);
