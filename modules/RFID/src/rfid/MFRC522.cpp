...
/*
 * Its a bit of a hack. But i'm lazy 😁
 */
class __Serial {
public:
    template<typename T>
...
byte MFRC522::PCD_ReadRegister(PCD_Register reg    ///< The register to read from. One of the PCD_Register enums.
    ...
    _spiClass->beginTransaction(_spiSettings);    // Set the settings to work with SPI bus
    digitalWrite(_chipSelectPin, LOW);            // Select slave
    _spiClass->transfer(0x80
                            | reg);                    // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
...
void MFRC522::PCD_ReadRegister(PCD_Register reg,    ///< The register to read from. One of the PCD_Register enums.
    ...
    byte address =
        0x80 | reg;                // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
...
MFRC522::StatusCode MFRC522::PCD_CommunicateWithPICC(byte command,        ///< The command to execute. One of the PCD_Command enums.
    ...
    byte errorRegValue =
        PCD_ReadRegister(ErrorReg); // ErrorReg[7..0] bits are: WrErr TempErr reserved BufferOvfl CollErr CRCErr ParityErr ProtocolErr
...
MFRC522::StatusCode MFRC522::PICC_REQA_or_WUPA(byte command,        ///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
    ...
    validBits =
        7;                                    // For REQA and WUPA we need the short frame format - transmit only 7 bits of the last (and only) byte. TxLastBits = BitFramingReg[2..0]
...
MFRC522::StatusCode MFRC522::PICC_Select(Uid *uid,            ///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
    ...
    byte
        bufferUsed;                // The number of bytes used in the buffer, ie the number of bytes to transfer to the FIFO.
    ...
    while (!uidComplete) {
        ...
        switch (cascadeLevel) {
        case 1: buffer[0] = PICC_CMD_SEL_CL1;
            uidIndex = 0;
            useCascadeTag = validBits && uid->size > 4;    // When we know that the UID has more than 4 bytes
            break;

        case 2: buffer[0] = PICC_CMD_SEL_CL2;
            uidIndex = 3;
            useCascadeTag = validBits && uid->size > 7;    // When we know that the UID has more than 7 bytes
            break;

        case 3: buffer[0] = PICC_CMD_SEL_CL3;
            uidIndex = 6;
...
MFRC522::StatusCode MFRC522::PCD_NTAG216_AUTH(byte *passWord, byte pACK[]) //Authenticate with 32bit password
{
    ...
    byte waitIRq = 0x30;    // RxIRq and IdleIRq
//	byte cmdBufferSize	= sizeo(cmdBuffer);
    byte validBits = 0;
    byte rxlength = 5;
 ...
 */
const std::string MFRC522::GetStatusCodeName(MFRC522::StatusCode code    ///< One of the StatusCode enums.
) {
 ...
 * @return const __FlashStringHelper *
 */
const std::string MFRC522::PICC_GetTypeName(PICC_Type piccType    ///< One of the PICC_Type enums.
) {
...
void MFRC522::PCD_DumpVersionToSerial() {
    ...
    switch (v) {
    case 0x88: Serial.println((" = (clone)"));
        break;
    case 0x90: Serial.println((" = v0.0"));
        break;
    case 0x91: Serial.println((" = v1.0"));
        break;
    case 0x92: Serial.println((" = v2.0"));
        break;
...
void MFRC522::PICC_DumpToSerial(Uid *uid    ///< Pointer to Uid struct returned from a successful PICC_Select().
    ...
    case PICC_TYPE_MIFARE_UL: PICC_DumpMifareUltralightToSerial();
        break;
    ...
    case PICC_TYPE_TNP3XXX: Serial.println(("Dumping memory contents not implemented for that PICC type."));
        break;

    case PICC_TYPE_UNKNOWN:
    case PICC_TYPE_NOT_COMPLETE:
    default: break; // No memory dump here
    }
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(Uid *uid,            ///< Pointer to Uid struct returned from a successful PICC_Select().
    ...
    for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
        ...
        if (firstInGroup) {
            // Print access bits
            Serial.print((" [ "));
            Serial.print((g[group] >> 2) & 1);
            Serial.print((" "));
            Serial.print((g[group] >> 1) & 1);
            Serial.print((" "));
            Serial.print((g[group] >> 0) & 1);
            Serial.print((" ] "));
            if (invertedError) {
